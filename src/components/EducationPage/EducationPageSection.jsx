...
Set REACT_APP_EDU_FETCH_ENDPOINT accordingly.`
        ...
        setError(
          e?.response?.data?.message || e.message || "Failed to load education data"
        );
      }
  ...
  async function handleSave(e) {
    ...
    try {
      const payload = {
        ...
        onlineCourseNames: data.onlineCourseNames.map((c) => c.trim()).filter(Boolean),
      ...
      console.log("[Education] PUT:", `${baseUrl}${EDU_UPDATE_ENDPOINT}`, payload);
  ...
  return (
    ...
    >
      ...
      <Field label="Heading" as="textarea" value={data.heading} onChange={(v) => updateField("heading", v)} />
      <ArrayField
      ...
      />
      <Field label="Mobile" value={data.mobile} onChange={(v) => updateField("mobile", v)} />
      <Field label="Email" value={data.email} onChange={(v) => updateField("email", v)} />
    </form>
  );
}
