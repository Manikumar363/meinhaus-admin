    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-2 mb-6">
          <button className="text-gray-600 hover:text-gray-800" onClick={onBack}>
        ...
        <div className="border-t border-gray-100 pt-6 text-center">
          ...
          <button className="bg-gray-800 text-white py-3 px-6 rounded-lg font-medium hover:bg-gray-900 transition-colors" onClick={() => setShowPopup(true)}>Create & Send Link</button>
        </div>
      </div>
    </div>
    {showPopup && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-lg w-full mx-4 relative flex flex-col overflow-y-auto" style={{maxHeight: '90vh'}}>
          <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl" onClick={() => setShowPopup(false)}>&times;</button>
          <h2 className="text-xl font-semibold mb-6 text-center">Send Job Link Details</h2>
          <div className="mb-6 flex flex-col md:flex-row justify-between text-sm gap-4">
            <div>
          ...
          <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1">Professional Trade Amount</label>
            <input type="text" className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50" value="$ 1850" readOnly />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1">Start Date <span className="text-xs text-gray-400">(The date from which the project will start.)</span></label>
            <div className="relative">
              <input type="text" className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50" placeholder="Select Date" />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
                <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
              </span>
            </div>
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1">Description <span className="text-xs text-gray-400">(For Professional)</span></label>
            <textarea className="w-full border border-gray-300 rounded-lg px-4 py-2 bg-gray-50" rows={3} placeholder="Enter description" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1">Images</label>
            <div className="flex gap-2 mb-2">
              <div className="relative w-16 h-16">
                <img src="https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=100&h=100&fit=crop&crop=center" alt="img1" className="w-full h-full object-cover rounded-lg" />
                <button className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs">×</button>
              </div>
              <div className="relative w-16 h-16">
                <img src="https://images.unsplash.com/photo-1581858726788-75bc0f6a952d?w=100&h=100&fit=crop&crop=center" alt="img2" className="w-full h-full object-cover rounded-lg" />
                <button className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs">×</button>
              </div>
              <div className="relative w-16 h-16">
                <img src="https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=100&h=100&fit=crop&crop=center" alt="img3" className="w-full h-full object-cover rounded-lg" />
                <button className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs">×</button>
              </div>
            </div>
            <button className="border border-gray-800 font-semibold rounded-lg px-3 py-2 text-gray-800 flex items-center gap-2 hover:bg-gray-50 mb-2">
              <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10V6a5 5 0 0110 0v4M12 14v2"/></svg>
              Upload Images
            </button>
          </div>
          <div className="mb-6 flex gap-4 items-center">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked readOnly className="accent-gray-800" /> Send Via Email
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" className="accent-gray-800" /> Send Via SMS
            </label>
          </div>
          <button className="w-full bg-gray-800 text-white py-3 rounded-lg font-medium hover:bg-gray-900 transition-colors mb-4" onClick={() => navigate(`/create-send`)} >Send Job Link</button>
          <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 text-sm">
            <div className="font-bold text-gray-700 mb-2">NOTE:</div>
            <ul className="list-disc pl-4 text-gray-700">
              <li>Send Email or Send SMS will notify all professionals for this service.</li>
              <li>The professionals are those who showed interest in this service during signup or the subscribed professionals who have bought this service.</li>
              <li>The professionals who are subscribed or non subscribed to the service will receive the link to the project and can apply for the job.</li>
            </ul>
          </div>
        </div>
      </div>
