...
// ENV (Create React App => REACT_APP_*, Vite => also map to import.meta.env if needed)
const API_BASE =
  process.env.REACT_APP_API_URL ||
  process.env.REACT_APP_API_BASE ||
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_API_URL) ||
  "";
// Optional env for token (else fallback to localStorage)
const STATIC_PAGES_TOKEN =
  process.env.REACT_APP_PAGES_TOKEN ||
  (typeof import.meta !== "undefined" && import.meta.env?.VITE_PAGES_TOKEN) ||
  "";

const LIST_ENDPOINT = "/pages";

function getToken() {
  // Use precomputed env token (avoids duplicate logic + ESLint warning)
...
const AboutPageSection = () => {
  ...
  const [editing, setEditing] = useState(null); // page obj or null
  ...
  const fetchPages = useCallback(async () => {
    ...
    if (!token) {
      setError('Missing bearer token (login first or set REACT_APP_PAGES_TOKEN).');
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  const handleSave = async (page) => {
    ...
    try {
      ...
      const body = isNew
        ? { title: page.title.trim(), content: page.content, status: page.status }
      ...
      if (isNew) {
        ...
        setPages(prev =>
          prev.map(p =>
            p.id === page.id ? { ...p, content: page.content, status: page.status } : p
          )
        );
      }
      setEditing(null);
